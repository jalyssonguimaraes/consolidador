...
export function Holdings({positions,closed,realized,movements,quotes}:{positions:Position[];closed:Position[];realized:Position[];movements:Movement[];quotes:Quote[]}){
 ...
 const realizedRows=realized.filter(p=>(p.sellQuantity||0)>0);
 const realizedTotals=realizedRows.reduce((a,p)=>{const d=p.realizedDetail;return {quantity:a.quantity+(p.sellQuantity||0),gross:a.gross+(d?.gross||0),cost:a.cost+(d?.lots||[]).reduce((s,l)=>s+(l.cost||0),0),fees:a.fees+(d?.fees||0),irrf:a.irrf+(d?.irrf||0)}},{quantity:0,gross:0,cost:0,fees:0,irrf:0});
 const realizedNet=realizedTotals.gross-realizedTotals.cost-realizedTotals.fees-realizedTotals.irrf;
 useEffect(()=>{
 ...
 return <div className="holdings">
  ...
  {realizedRows.length>0&&<section className="surface closed-holdings"><h3>Vendas apuradas</h3><p className="note">Toda venda gera resultado realizado por FIFO, mesmo quando ainda existe saldo do ativo. Posições zeradas aparecem junto com vendas parciais.</p><div className="closed-list"><Table><TableHeader><TableRow>{['Ativo','Classe','Quantidade vendida','Venda bruta','Custo dos lotes','Taxas','IRRF','Resultado líquido'].map(s=><TableHead key={s}>{s}</TableHead>)}</TableRow></TableHeader><TableBody>{realizedRows.map(p=>{const d=p.realizedDetail;const cost=(d?.lots||[]).reduce((s,l)=>s+(l.cost||0),0);const net=(d?.gross||0)-cost-(d?.fees||0)-(d?.irrf||0);return <TableRow key={key(p)} className="holding-row" onClick={()=>open(p)}><TableCell><b>{p.asset}</b><small>{p.quantity===0?'Posição zerada':'Venda parcial · ver lotes'}</small></TableCell><TableCell>{p.category}</TableCell><TableCell className="numeric">{units(p.sellQuantity||0)}</TableCell><TableCell className="numeric">{currency(d?.gross||0)}</TableCell><TableCell className="numeric">{currency(cost)}</TableCell><TableCell className="numeric">{currency(d?.fees||0)}</TableCell><TableCell className="numeric">{currency(d?.irrf||0)}</TableCell><TableCell className={'numeric '+(net<0?'negative':'positive')}>{currency(net)}</TableCell></TableRow>})}<TableRow className="totals-row"><TableCell><b>Total realizado</b></TableCell><TableCell>—</TableCell><TableCell className="numeric"><b>{units(realizedTotals.quantity)}</b></TableCell><TableCell className="numeric"><b>{currency(realizedTotals.gross)}</b></TableCell><TableCell className="numeric"><b>{currency(realizedTotals.cost)}</b></TableCell><TableCell className="numeric"><b>{currency(realizedTotals.fees)}</b></TableCell><TableCell className="numeric"><b>{currency(realizedTotals.irrf)}</b></TableCell><TableCell className={'numeric '+(realizedNet<0?'negative':'positive')}><b>{currency(realizedNet)}</b></TableCell></TableRow></TableBody></Table></div></section>}

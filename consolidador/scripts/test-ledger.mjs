...
assert.equal(source.notes.length,98);
const model=consolidate(source.notes);
assert.equal(model.movements.length,147);
...
// Correção de 2026-09-25 (data/source.json.dataCorrections): 7 notas "Investimento programado"
// da aba Tesouro Direto estavam ausentes da planilha original, causando resgates sem lote de
// compra correspondente para Tesouro Selic 2027. Com as notas repostas, esse gap não deve mais
// existir — só resta o gap real e distinto de ITSA4 (venda/transferência a conferir).
const treasuryGap=model.issues.find(i=>i.message.includes('Tesouro Selic 2027'));
assert.equal(treasuryGap,undefined,'gap de Tesouro Selic 2027 deveria ter sido corrigido pela reposição das notas');
const stockGap=model.issues.find(i=>i.message.includes('ITSA4'));
assert(stockGap);
assert.match(stockGap.message,/excede o saldo consolidado/);
...
console.log(JSON.stringify({notes:98,movements:147,feeReconciliations:98*feeKeys.length,issues:model.issues.length,tests:'passed'}));

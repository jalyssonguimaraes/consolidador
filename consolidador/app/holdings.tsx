...
export function Holdings({positions,closed,movements,quotes}:{positions:Position[];closed:Position[];movements:Movement[];quotes:Quote[]}){
 ...
 return <div className="holdings">
  ...
  {closed.length>0&&<section className="surface closed-holdings"><h3>Operações encerradas</h3><p className="note">Posições zeradas e resultado por FIFO. O resultado é gerencial: taxas e IRRF são mostrados separadamente e não substituem a apuração fiscal.</p><div className="closed-list"><Table><TableHeader><TableRow>{['Ativo','Classe','Quantidade vendida','Venda bruta','Custo dos lotes','Taxas','IRRF','Resultado líquido'].map(s=><TableHead key={s}>{s}</TableHead>)}</TableRow></TableHeader><TableBody>{closed.map(p=>{const d=p.realizedDetail;const cost=(d?.lots||[]).reduce((s,l)=>s+(l.cost||0),0);const net=(d?.gross||0)-cost-(d?.fees||0)-(d?.irrf||0);return <TableRow key={key(p)} className="holding-row" onClick={()=>open(p)}><TableCell><b>{p.asset}</b><small>Ver lotes e operações</small></TableCell><TableCell>{p.category}</TableCell><TableCell className="numeric">{units(p.sellQuantity||0)}</TableCell><TableCell className="numeric">{currency(d?.gross||0)}</TableCell><TableCell className="numeric">{currency(cost)}</TableCell><TableCell className="numeric">{currency(d?.fees||0)}</TableCell><TableCell className="numeric">{currency(d?.irrf||0)}</TableCell><TableCell className={'numeric '+(net<0?'negative':'positive')}>{currency(net)}</TableCell></TableRow>})}</TableBody></Table></div></section>}

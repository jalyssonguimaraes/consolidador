...
export async function POST(req:Request){
 ...
 try{
  ...
  const assets=[...new Map(positions.map(p=>[p.asset,p])).values()];let updated=0;const failed:string[]=[];
  ...
  if(recent&&Date.now()-Date.parse(recent.updated)<60000)return Response.json({message:'As cotações foram consultadas há menos de um minuto. Mantidos os últimos valores.'});
  for(const p of assets){
   ...
   try{
    ...
    if(!r.ok){failed.push(p.asset);if(r.status===429)break;continue;}
    ...
    if(!Number.isFinite(price)||price<=0||Number.isNaN(Date.parse(at))||(!treasury&&item?.changed===true)){failed.push(p.asset);continue;}
    const payload={asset:p.asset,price,at,name:v?.longName||v?.shortName||p.asset,symbol};
    await database().prepare('INSERT INTO quotes(id,owner,payload,updated) VALUES(?,?,?,?) ON CONFLICT(id) DO UPDATE SET payload=excluded.payload,updated=excluded.updated').bind(user.userId+':'+p.asset,user.userId,JSON.stringify(payload),new Date().toISOString()).run();updated++;
